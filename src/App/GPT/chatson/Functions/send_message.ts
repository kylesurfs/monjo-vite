...
export async function send_message(
  ...
  CC: IChatContext,
  handleCompletionContent?: (content: string) => void, //-- KYLE --//
  ...
  const user_db_id = 'k1k1'; //--KYLE: TODO - create random generator for db_id --//
  ...
  await fetchEventSource(`${VITE_ALB_BASE_URL}/openai/v1/chat/completions`, {
    ...
    onmessage(event) {
      ...
      //-- SSE completion content chunks --//
        ...
        console.log(uriDecodedData); //--KYLE --//
        if (handleCompletionContent) {
          handleCompletionContent(uriDecodedData);
        }
    ...
    onclose() {
      ...
      console.log('Final completion content: ', completion_content);
      if (handleCompletionContent) {
        handleCompletionContent(completion_content);
      }
