...
export default function ChatSession() {
  ...
  const { updateTrigger, setUpdateTrigger } = useChatContext(); //--- KYLE: Using to try to force a component re-render during streaming ----//
  ...
  //--- KYLE: Using to try to force a component re-render during streaming ----//
  useEffect(() => {
    // This effect runs whenever updateTrigger changes, indicating a new message has been received.
    // The specific logic here can be an empty block, as the sole purpose is to trigger re-renders.
  }, [updateTrigger]);
  ...
  const regenerateResponse = async () => {
    ...
    // const accessToken = generateDummyToken();
    if (CC.rowArray) {
    ...
    <div id='chat-session-tld' className='flex max-h-full min-h-full flex-col'>
      ...
      >
        ...
        <div className='flex flex-row justify-center'>
          ...
          >
            {/* KYLE -- Begin commented out section (just to keep UI leaner for teachers) */}
            {/* <div
                id='control-bar-LHS'
            ...
            {/* KYLE -- END commented out section */}
            ...
            <div className='relative mt-2 w-full max-w-prose rounded-lg shadow-md'>
              ...
              {CC.completionRequested ? (
                ...
                <button
                  ...
                  onClick={() => {
                    submitPromptHandler();
                    setShowCompletion(true); // Trigger showing the CompletionDisplay (KYLE: Monjo)
                  }}
