...
export const ConversationsRow = (
  ...
  let CC_kyle = useChatContext();
  const { accessToken } = CC_kyle; //--- KYLE: dummy access token --//

  //-- START: Original function with auth --//
  ...
  //-- END: Original function with auth --//

  //-- START: NEW (Kyle) function with auth --//
  // const confirmDeleteHandler = async (event: React.MouseEvent) => {
  //   event.stopPropagation();
  //   // Mock or directly call the function without an access token
  //   try {
  //     await delete_conversation_and_messages(row._id, CC, navigate);
  //   } catch (err) {
  //     if (err instanceof AxiosError) {
  //       axiosErrorToaster(err, 'Delete Conversation');
  //     } else if (err instanceof Error) {
  ...
  //-- END: NEW (Kyle) function with auth --//
